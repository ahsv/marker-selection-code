...
def rank_genes_groups(
    ...
    **kwds : keyword parameters
    ...
    print("After select_groups, groups_order = {}".format(groups_order), flush=True)
    ...
    if method in {'t-test', 't-test_overestim_var'}:
        ...
        X = adata_comp.X
        print("Done making multiple copies of the data as part of the logreg method", flush=True)

        clf = LogisticRegression(**kwds)
        clf.fit(X, adata_copy.obs[groupby].cat.codes)
        print("Number of iterations taken: {}".format(clf.n_iter_), flush = True)
        ...
        # If no reference group exists, ranking needs only to be done once (full mask)
            ...
            #print(str(scores[0][2000]) + " " + str(scores[1][2000]))
    ...
    print(groups_order_save, flush=True)
    #print(rankings_gene_scores[0:10], flush=True)
    #print([n for n in rankings_gene_scores])
    #prinit([(rn, 'float32') for rn in groups_order_save])
